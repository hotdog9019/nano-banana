...
def _prepare_summary_for_flags(df: pd.DataFrame):
    """
    Подготавливает summary для compute_quality_flags.
    Возвращает объект с нужными атрибутами.
    """
    from .core import build_summary, summarize_dataset
    
    # Сначала получаем стандартный summary
    dataset_summary = summarize_dataset(df)
    
    # Затем получаем расширенный summary из build_summary
    extended_summary = build_summary(df)
    ...
    # Атрибуты из dataset_summary
    result.n_rows = dataset_summary.n_rows
    result.n_cols = dataset_summary.n_cols
    
    # Атрибуты из extended_summary
    result.n_const_cols = getattr(extended_summary, "n_const_cols", 0)
    result.max_cat_card = getattr(extended_summary, "max_cat_card", 0)
